 ...
 */(context: Context) : View(context) {
    ...
    fun setCurlSpeed(mCurlSpeed: Int) {
     ...
     * @param pages
     */
    fun setCurlView(pages: List<Int?>) {

        // LEGACY PAGE HANDLING!

        // Create pages
        mPages = ArrayList()
        for (image in pages.indices) {
            mPages!!.add(BitmapFactory.decodeResource(resources, pages[image]!!))
        }
...
//    }
    ...
    private fun init(context: Context) {
        ...
        mMovement = Vector2D(0.toFloat(), 0.toFloat())
        mFinger = Vector2D(0.toFloat(), 0.toFloat())
        mOldMovement = Vector2D(0.toFloat(), 0.toFloat())
    ...
    fun ResetClipEdge() {
        ...
        mA = Vector2D(mInitialEdgeOffset.toFloat(), 0.toFloat())
        mB = Vector2D(this.width.toFloat(), this.height.toFloat())
        mC = Vector2D(this.width.toFloat(), 0.toFloat())
        mD = Vector2D(0.toFloat(), 0.toFloat())
        mE = Vector2D(0.toFloat(), 0.toFloat())
        mF = Vector2D(0.toFloat(), 0.toFloat())
        mOldF = Vector2D(0.toFloat(), 0.toFloat())

        // The movement origin point
        mOrigin = Vector2D(this.width.toFloat(), 0.toFloat())
    }
            ...
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    if (mOldMovement!!.x > width shr 1) {
                        ...
                        // Set new movement
                        mMovement!!.x =
                            if (IsCurlModeDynamic()) (width shl 1).toFloat() else width.toFloat()
                        mMovement!!.y = mInitialEdgeOffset.toFloat()
                    }
                }
